# Getting data which takes a while to run for recip model 
...
conductivity_summer_indo <- rast('./Envs_Indo/conductivity_summer_18_indo.tif')
for(i in 1:dim(conductivity_summer_indo)[3]) {
  conductivity_summer_indo[[i]] <- interpolateRaster(conductivity_summer_indo[[i]])
}
current_north_december_indo <- 
  rast('./Envs_Indo/env_indo_3D_current_north_december_interpolated.tif')
current_north_june_indo <- rast('./Envs_Indo/current_north_june_indo.tif')
for(i in 1:dim(current_north_june_indo)[3]) {
  current_north_june_indo[[i]] <- interpolateRaster(current_north_june_indo[[i]])
}
current_east_december_indo <- rast('./Envs_Indo/current_east_december_indo.tif')
for(i in 1:dim(current_east_december_indo)[3]) {
  current_east_december_indo[[i]] <- interpolateRaster(current_east_december_indo[[i]])
}
current_east_june_indo <- rast('./Envs_Indo/current_east_june_indo.tif')
for(i in 1:dim(current_east_june_indo)[3]) {
  current_east_june_indo[[i]] <- interpolateRaster(current_east_june_indo[[i]])
}
...
temperature_winter_indo <- rast('./Envs_Indo/temperature_winter_18_indo.tif')
for(i in 1:dim(temperature_winter_indo)[3]) {
  temperature_winter_indo[[i]] <- interpolateRaster(temperature_winter_indo[[i]])
}
salinity_winter_indo <- rast('./Envs_Indo/salinity_winter_18_indo.tif')
for(i in 1:dim(salinity_winter_indo)[3]) {
  salinity_winter_indo[[i]] <- interpolateRaster(salinity_winter_indo[[i]])
} 
salinity_summer_indo <- rast('./Envs_Indo/salinity_summer_18_indo.tif')
for(i in 1:dim(salinity_summer_indo)[3]) {
  salinity_summer_indo[[i]] <- interpolateRaster(salinity_summer_indo[[i]]) 
}
density_winter_indo <- rast('./Envs_Indo/density_winter_18_indo.tif')
for(i in 1:dim(density_winter_indo)[3]) {
  density_winter_indo[[i]] <- interpolateRaster(density_winter_indo[[i]])
}
density_summer_indo <- rast('./Envs_Indo/density_summer_18_indo.tif')
for(i in 1:dim(density_summer_indo)[3]) {
  density_summer_indo[[i]] <- interpolateRaster(density_summer_indo[[i]])
}
...
writeRaster(envs_final, filename = './Envs_Indo/interpolated_indo_final_envs_recip.tif',
            overwrite = T)
...
writeRaster(current_east_june, filename = './Envs_Indo/current_east_june_mozam_interp.tif')

for (i in 1:dim(current_east_december)[3]) {
  current_east_december[[i]] <- interpolateRaster(current_east_december[[i]])
}
current_east_december <- mask(current_east_december, land, inverse = T)
writeRaster(current_east_december, 
            filename = './Envs_Indo/current_east_december_mozam_interp.tif')
